...
Board.prototype.toggleTutorialButtons = function() {
  ...
  function nextPreviousClick() {
    if (counter === 1) {
      document.getElementById("tutorial").innerHTML = `<h3>Hiii, đây là Pathfinding Algorithms Visualizer!</h3><h6>Đoạn hướng dẫn này sẽ cho bạn cái nhìn tổng quan về web app này.</h6><p>Nếu cảm thấy không cần thiết, click <b>"Bỏ qua hướng dẫn"</b> bên dưới. Còn không thì nhấn <b>"Tiếp theo"</b> để bắt đầu nhaa !</p><div id="tutorialCounter">1/9</div><img id="mainTutorialImage" src="public/styling/c_icon.png"><button id="nextButton" class="btn btn-default navbar-btn" type="button">Tiếp theo</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Quay lại</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Bỏ qua hướng dẫn</button>`
    } else if (counter === 2) {
      document.getElementById("tutorial").innerHTML = `<h3>Thuật toán Tìm đường đi ngắn nhất</h3><h6>Nói dễ hiểu thì như tên gọi, những thuật toán này sẽ đoạn đường ngắn nhất nối 2 điểm lại với nhau.</h6><p> Và ứng dụng này sẽ biểu diễn trực các thuật toán đó, ngoài một số thuật toán chắc các bạn đã được học trong môn "Cấu trúc dữ liệu & Giải thuật", mình có bổ sung thêm vài thuật toán khác nữa. Tất cả các thuật toán sẽ được biểu diễn thực tế dưới dạng lưới chứ không phải sơ đồ như các bạn đã học. <br/> <b>Lưu ý,</b> những khúc ôm cua 90° sẽ có trọng số là 1 và những chuyện động giữa 2 điểm cũng có trọng số là 1.</p><div id="tutorialCounter">${counter}/9</div><img id="mainTutorialImage" src="public/styling/path.png"><button id="nextButton" class="btn btn-default navbar-btn" type="button">Tiếp theo</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Quay lại</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Bỏ qua hướng dẫn</button>`
    } else if (counter === 3) {
      ...
      document.getElementById("tutorial").innerHTML = `<h3>Chèn thêm Bomb</h3><h6>Click vào button <b>"Add Bomb"</b> để thêm bomb.</h6><p>Thêm bomb sẽ thay đổi quy trình giải thuật của thuật toán. Đầu tiên, nó sẽ tìm quả bomb trước tiên, sau đó mới tìm Điểm cuối. <b>Lưu ý,</b> thuật toán <i>Bidirectional Swarm</i> không hỗ trợ bomb.</p><img id="secondTutorialImage" src="public/styling/bomb.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Tiếp theo</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Quay lại</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Bỏ qua hướng dẫn</button>`
    } else if (counter === 7) {
      document.getElementById("tutorial").innerHTML = `<h3>Kéo thả các nút</h3><h6>Có thể click và kéo các Điểm bắt đầu, Điểm cuối và Bomb để di chuyển chúng.</h6><p>Lưu ý rằng chỉ có thể kéo thả sau khi giải thuật chạy xong. <br/> Thao tác này giúp bạn có thể xem các đường đi khác ngay lập tức.</p><img src="public/styling/dragging.gif"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Tiếp theo</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Quay lại</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Bỏ qua hướng dẫn</button>`
    } else if (counter === 8) {
      document.getElementById("tutorial").innerHTML = `<h3>Biểu diễn thuật toán</h3><h6>Sử dụng các button trên thanh công cụ để biểu diễn thuật toán và hơn thế nữa !</h6><p>Ứng dụng hỗ trọ một số chức năng thêm như Xoá đường đi hiện tại, Xoá các nút Tường và Trọng số, Chỉnh tốc độ giải thuật hoặc Reset lại như ban đầu. Hãy lựa chọn đúng lúc đúng thời điểm nhaa !</p><br/><img id="secondTutorialImage" width="650px" src="public/styling/navbar.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Tiếp theo</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Quay lại</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Bỏ qua hướng dẫn</button>`
    } else if (counter === 9) {
      document.getElementById("tutorial").innerHTML = `<h3><br/><br/><br/><br/>Enjoy and have fun!</h3><h6>Hi vọng qua ứng dụng này các bạn có thể hiểu hơn về các giải thuật Tìm đường.</h6><p>Nếu có cấu hỏi gì có thể liên hệ <a href="https://meokisama.github.io">tớ</a>. Thân chào và cảm ơn các bạn đã theo dõi !</p><div id="tutorialCounter">${counter}/9</div><button id="finishButton" class="btn btn-default navbar-btn" type="button">Xong</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Quay lại</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Bỏ qua hướng dẫn</button>`
...
Board.prototype.toggleButtons = function() {
  ...
  if (!this.buttonsOn) {
    ...
    document.getElementById("startButtonBidirectional").onclick = () => {
      document.getElementById("startButtonStart").innerHTML = '<button id="actualStartButton" class="btn btn-default navbar-btn" type="button">Biểu diễn Bidirectional Swarm!</button>'
